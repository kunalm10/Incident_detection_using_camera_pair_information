...
time_to_run_every_x_seconds = 5  # the database will update in every 5 seconds
...
def get_all_cameras():
    ...
    return [cam_id1, cam_id2]


def get_neighbor_cameras(reference_camera_id):
    result = []
    if reference_camera_id == cam_id1:
        result.append(cam_id2)
        return result
    elif reference_camera_id == cam_id2:
        result.append(cam_id1)
        return result


def get_current_direction(camera_id):

    if camera_id == cam_id1:
        return direction1
    elif camera_id == cam_id2:
        return direction2


def get_reference_and_neighbor_camera_valid_direction_pair(reference_camera_id, neighbor_camera_id):
    result = []
    if reference_camera_id == cam_id1 and neighbor_camera_id == cam_id2:
        result_dict = dict()
        result_dict['reference_to_neighbor_direction'] = direction1
        result_dict['neighbor_to_reference_direction'] = direction2
        result_dict['direction_match_flag'] = 'SAME'
        result.append(result_dict)
        return result

    elif reference_camera_id == cam_id2 and neighbor_camera_id == cam_id1:
        result_dict = dict()
        result_dict['reference_to_neighbor_direction'] = direction2
        result_dict['neighbor_to_reference_direction'] = direction1
        result_dict['direction_match_flag'] = 'SAME'
        result.append(result_dict)
        return result
...
def get_flow_rate(camera_id, direction, side):
    ...
    #         "WHERE cfr.camera_road_id = ld.cam_road_id and ld.lane_id = td.lane_id and cfr.camera_id = %i " \
    #         "and cfr.direction = '" + str(direction) + "' and ld.lane_annotation > 0" % (camera_id)
    query = "SELECT distinct(avg_traffic_flow) FROM traffic_status td, lane_details ld, cam_facing_road cfr " \
...
def update_jam_status_current(datetime,reference_camera_id, neighbor_camera_id, reference_cam_current_direction, neighbor_cam_current_direction, JAM_STATUS_left_lane , JAM_STATUS_right_lane):
    ...
    cursor.execute(query, (str(reference_cam_current_direction), str(neighbor_cam_current_direction), str(JAM_STATUS_right_lane), str(datetime),
                   str(JAM_STATUS_left_lane), int(reference_camera_id), int(neighbor_camera_id)))
    cnx.commit()


def insert_jam_status_history(datetime, reference_camera_id, neighbor_camera_id, reference_cam_current_direction, neighbor_cam_current_direction, JAM_STATUS_left_lane , JAM_STATUS_right_lane):
    ...
    query = "INSERT INTO jam_status_history (`reference_cam_id`, `neighbor_cam_id`, `road_name`, `reference_current_direction`, " \
            ...
            "%s, %s, %s, %s, %s, %s, %s, %s)"
    cursor.execute(query, (str(reference_camera_id), str(neighbor_camera_id), str(road_name), str(reference_cam_current_direction), str(neighbor_cam_current_direction),
                str(JAM_STATUS_left_lane) , str(JAM_STATUS_right_lane), str(datetime)))
    cnx.commit()


def flow_rate_compare(reference_cam_flow_rate_right, reference_cam_flow_rate_left,neighbor_cam_flow_rate_right, neighbor_cam_flow_rate_left, direction_match_flag):
...
#this function is just for debugging purpose, where ever this function is used, that part can be commented and the code will still run smooth.
...
#this function is just for debugging purpose, where ever this function is used, that part can be commented and the code will still run smooth.
def direction_illustration_different_direction(reference_camera_id, neighbor_camera_id, reference_cam_current_direction, neighbor_cam_current_direction):
    ...
    print("current orientation of cameras\n"
          ...
          "---------------------------------------------\n"
          % (
          reference_camera_id, neighbor_camera_id, reference_direction_illustration, neighbor_direction_illustration))
...
if __name__ == '__main__':
    while True:
        ...
        reference_cameras_list = get_all_cameras()
        ...
        for index, reference_camera_id in enumerate(reference_cameras_list):
            ...
            print("Reference_cam_id = ", reference_camera_id )
            # Make a list of dictonaries which contain reference_to_neighbor_direction, and neighbor_to_reference_direction, direction flag
            ...
            print('reference_camera_current_direction = ', reference_cam_current_direction, '\n')

            #Check if reference camera current direction is available to us, if not do not make any comparisions, this camera will be skipped
            if reference_cam_current_direction != None:
                ...
                for neighbor_camera_id in neighbor_cameras:
                    ...
                    if neighbor_cam_current_direction != None:
                        neighbor_camera_details = get_reference_and_neighbor_camera_valid_direction_pair(reference_camera_id, neighbor_camera_id)
                        ...
                        for count, each_neighbor_camera in enumerate(neighbor_camera_details):
                            ...
                            print('Neighbor_cam_to_Reference_cam_direction, Reference_cam_to_Neighbor_cam_direction = ', neighbor_to_reference_direction, reference_to_neighbor_direction)
                            print('    Neighbor_cam_Current_direction,          Reference_cam_Current_direction     = ', neighbor_cam_current_direction, reference_cam_current_direction, '\n');
                            ...
                            if reference_cam_current_direction == reference_to_neighbor_direction and neighbor_cam_current_direction == neighbor_to_reference_direction:
                                print('--------------------------current directions match one of the DIRECTION PAIR(S) in which flow rate comparision is POSSIBLE-------------------------- \n')
                                print('-------------------------------------------fetching flow rate from database to determine the status of JAM-------------------------------------------\n')

                                #fetch the flow rates of neighbor camera and reference camera
                                neighbor_cam_flow_rate_right = get_flow_rate(neighbor_camera_id, neighbor_cam_current_direction, 'right')
                                neighbor_cam_flow_rate_left = get_flow_rate(neighbor_camera_id, neighbor_cam_current_direction, 'left')

                                reference_cam_flow_rate_right = get_flow_rate(reference_camera_id, reference_cam_current_direction, 'right')
                                reference_cam_flow_rate_left = get_flow_rate(reference_camera_id, reference_cam_current_direction, 'left')

                                print('flow rate of reference camera right,left = ', reference_cam_flow_rate_right, reference_cam_flow_rate_left)
                                print('flow rate of neighbor camera right,left = ', neighbor_cam_flow_rate_right, neighbor_cam_flow_rate_left, '\n')
                                ...
                                if neighbor_cam_flow_rate_left != None and neighbor_cam_flow_rate_right != None and \
                                    ...
                                    JAM_STATUS_right_lane, JAM_STATUS_left_lane = flow_rate_compare(reference_cam_flow_rate_right, reference_cam_flow_rate_left, neighbor_cam_flow_rate_right, neighbor_cam_flow_rate_left, direction_match_flag)
                                    update_jam_status_current(current_datetime, reference_camera_id, neighbor_camera_id, reference_cam_current_direction, neighbor_cam_current_direction, JAM_STATUS_left_lane , JAM_STATUS_right_lane)
                                    insert_jam_status_history(current_datetime, reference_camera_id, neighbor_camera_id, reference_cam_current_direction, neighbor_cam_current_direction, JAM_STATUS_left_lane , JAM_STATUS_right_lane)

                                    print('JAM STATUS of left lane of ROAD which has camera', reference_camera_id,'monitoring', reference_cam_current_direction, 'direction = ', JAM_STATUS_left_lane )
                                    print('JAM STATUS of right lane of ROAD which has camera', reference_camera_id, 'monitoring', reference_cam_current_direction, 'direction = ', JAM_STATUS_right_lane)
                                else:
                                ...
                                print('-----------current direction does not match the STORED VALID DIRECTION PAIR(S) in which we can compare flow rate----------\n'
                                      '-------------------------------------------so flow rate comparision is not possible------------------------------------------ \n')
                    else:
                        print("CAN'T DETERMINE STATUS of the ROAD because can't read the DIRECTION of the Neighbor Camera\n")
